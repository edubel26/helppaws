    ...
    <img class="logo-menu"
     ...
     die('Hay un error corriendo en la consulta o datos no encontrados!!! [' . $db->error . ']');
            ...
            while($row = $result->fetch_assoc())
            {
                ...
                $primer_nombre=stripslashes($row["primer_nombre"]);
                $segundo_nombre=stripslashes($row["segundo_nombre"]);
                $tercer_nombre=stripslashes($row["tercer_nombre"]); 
                $primer_apellido=stripslashes($row["primer_apellido"]);
                $segundo_apellido=stripslashes($row["segundo_apellido"]);
...
echo"<h2 class='titulo'>jojo</h2>";
echo"<form action='b-actualizar_mascota.php' method='post' enctype='multipart/form-data' name='form1' id='form1'>";
echo"<input required class='controls' type='hidden' name='tipo_doc' value= '$tipo_doc'>";
...
echo"<input class='botons' type='submit' value='Actualizar mascota'>";   
 ?>
        ...
        <center><br/><a href="b-mi_perfil.php">Ir atras</a></center>
    </body>
